...
function start(){
    ...
    for(var i = 0; i < 8; i+=2){
        enemies.push(new sprite.Enemy(i,i))
    }
    for(var i = 0; i < 8; i+=2){
        enemies.push(new sprite.Enemy(i,i-7))
    }
    for(var i = 0; i < 8; i+=2){
        enemies.push(new sprite.Enemy(i,i-14))
    }
    for(var i = 0; i < 8; i+=2){
        enemies.push(new sprite.Enemy(i,i-21))
    }
...
function mainLoop(){
    ...
    enemies.forEach(function(enemy){
        enemy.tick();
        hit=hit||rectangularCollisionTest(player,enemy);
    });
    
    hit=hit||rectangularCollisionTest(player, stewart);
    ...
    if(ticks%levelGap === 0){
        level++;
        if(level%5===0){
            powerupLevel = level/5
            powerup.powerupLevel=powerupLevel;
            powerup.active=true;
            powerup.x=powerupLevel *100 + 77;
            powerup.y=powerupLevel *50 + 77;
            powerup.w=15
            powerup.h=15
