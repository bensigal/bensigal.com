class Player{
    ...
    tick(dt){
        ...
        this.x += this.velocity.x*dt/16;
        this.y += this.velocity.y*dt/16;
        ...
        if(keyboard.down && keyboard.right && this.index!=1 || keyboard.s && keyboard.d && this.index!= 2){
            this.velocity.y += this.speed/Math.sqrt(2)*dt/16;
            this.velocity.x += this.speed/Math.sqrt(2)*dt/16;
    ...
    draw(){
        ...
        ctx.fillText("SF",this.x, this.y+5);
    }
...
class PointDestroyer{
    ...
    tick(){
        ...
        this.angle = this.rps*Math.PI*2*(new Date().getTime() - startTime)/1000 +this.startingIndex;
    ...
    draw(){
        ...
        ctx.font="12px Arial";
        ctx.textAlign="center";
        ctx.fillText("Mom", this.x, this.y)
    }
...
class Enemy{
    ...
    tick(){
        ...
        this.x2 = this.x - Math.cos(this.angle)*100;
        this.y2 = this.y - Math.sin(this.angle)*100;
